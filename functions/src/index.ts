...
/**
 * Calculate the results of a double match if an entry was found in the
 * GameRequestsDoubles database
 */
export const calculateDoubleMatch =
  functions.firestore
      .document("GameRequestsDoubles/{userId}")
      .onCreate((snap) =>{
 ...
 * @param {boolean} won Whether or not the player won the game
 * @param {number} points Points of this player
 * @param {number} wins Total wins of this player
 * @param {number} losses Total losses of this player
 * @param {number} streak Current winstreak of this player
 * @param {number} maxStreak Longest recorded winstreak of this player
 * @param {number} pointGain Points this player gained most recently
 */
        class Player {
          name:string;
          won:boolean;
 ...
 * @param {string} name transfers name of player
 * @param {boolean} won if player won most recent game (transferred to class)
 */
            ...
            .then((doc) =>{
              if (doc !== undefined) {
                ...
                if (data !== undefined) {
                  ...
                  console.log(
                      "DecayFactor: "+ decayCalculationFactor+
              " cutoffgames: " + decayCutoffGames +
        ...
        Promise.all(promises)
            ...
            .then(() =>{
              ...
              console.log(
                  "Winneraddition: "+ winnerAddition+
          " LoserDeduction: "+loserDeduction);

              // winner addition and loser deduction will be applied to the
              // players here each player will gain/lose the amount of
              // points calculated.
              // This effectively gives a doubles match the same score per
              // player as a singles match. the points are also normalised
              // accross the teams the person with more points pays
              // more in case of a loss the person with more gains
              // less in case of a win

              // Below is some logging of the split among winner and loser
              const winner1Addition =
          Math.round(winnerAddition * (winners[1].points / winningTeamPoints));
              const winner2Addition =
          Math.round(winnerAddition * (winners[0].points / winningTeamPoints));
              const loser1Addition =
          Math.round(loserDeduction * (losers[0].points / losingTeamPoints));
              const loser2Addition =
          Math.round(loserDeduction * (losers[1].points / losingTeamPoints));
              console.log(
                  "Winner split: Winner1: " + winner1Addition +
          " Winner2: "+ winner2Addition);
              console.log(
                  "Loser split: Loser1: " + loser1Addition +
          " Loser2: "+ loser2Addition);
            ...
            " Won: " + player.won);
              ...
              // calculate decay
              helper.decayPreparation(
                  innerPromises,
                  players,
                  gamesPlayed,
                  decayCalculationFactor,
                  decayCutoffGames,
                  decayPoints);
            })
            .catch((err) => console.log(err));
...
export const playerAdded =
  functions.firestore
      .document("PlayerAddRequests/{userId}")
      .onCreate((snap) =>{
        const snapData = snap.data();
        if (snapData === undefined) {
          return null;
...
export const calculateSingleMatch =
  functions.firestore
      .document("GameRequests/{userId}")
      .onCreate((snap) =>{
        const snapData = snap.data();
        if (snapData === undefined) {
          return null;
        ...
        // decayCutoffGames - number of games from which a player has to be
        // absent in order to be hit by decay decayPoints - average number
        // of points given by each player hit by decay
        ...
        Promise.all(promises)
            ...
            .then(() =>{
              if (
                winnerLastOpponent === loser ||
                loserLastOpponent === winner) {
                return;
              }
          ...
          " LoserDeduction: "+loserDeduction);
              console.log(
                  "Loser is champion: " + loserIsChampion +
          " Winner is champion: "+ winnerIsChampion);
              ...
              winnerStreak += 1;
              winnerMaxStreak =
          winnerStreak > winnerMaxStreak ? winnerStreak : winnerMaxStreak;
              ...
              // finally we need to do decay preparation and check if we
              // actually start calculating decay
              helper.decayPreparation(
                  innerPromises,
                  players,
                  gamesPlayed,
...
export const initialiseDatabase =
  functions.firestore
      ...
      .onCreate((snap) =>{
        ...
        const playerAddRequests =
          admin.firestore().collection("PlayerAddRequests");
