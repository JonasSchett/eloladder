...
/**
 * Calculate decay.
 * @param {FirebaseFirestore.CollectionReference} players List of players.
 * @param {number} decayCutoffGames Number of games before decay kicks in.
 * @param {number} decayPoints Number of decay points.
 */
export function calculateDecay(
  ...
  promises.push(decayPlayers.get()
      .then((decayPlayersData) =>{
        decayPlayersData.forEach((player) =>{
          // only set decayplayers to players who are not frozen and
          // who have more than 1000 points
  ...
  promises.push(gainingPlayers.get()
      .then((gainingPlayersData) =>{
        ...
        // only set gaining players to players who are not frozen and
        // who have less than 2000 points
          if (player.data().points < 2000 && !player.data().frozen) {
            gainingPlayersArray.push({
              totalDecay: player.data().totalDecay,
              name: player.data().name,
              notPlayedForInverted:
                decayCutoffGames - player.data().notPlayedFor,
              currentPoints: player.data().points,
              scorePercentage: 0,
              notPlayedForPercentage: 0,
              fullPercentage: 0,
  ...
  Promise.all(promises)
      .then(() =>{
        ...
        console.log("DecayPoints: " + decayPoints +
          " DecayCutoff: "+ decayCutoffGames);
        ...
        for (const decayPlayer of decayPlayersArray) {
        // calculating the decay like this will increase the decay with the
        // factor of not having played, while at the same time decreasing the
        // decay with the distance the player is to 1000 points
          decayPlayer.scorePercentage = (decayPlayer.currentPoints - 1000) /
            totalPointsLess1000;
          decayPlayer.notPlayedForPercentage = decayPlayer.notPlayedFor /
            totalNotPlayedFor;
          decayPlayer.fullPercentage = decayPlayer.scorePercentage *
            decayPlayer.notPlayedForPercentage;
        ...
        for (const gainingPlayer of gainingPlayersArray) {
          // the closer a player is to 2000 the less points they will gain
          // from decay
          gainingPlayer.scorePercentage =
            (2000 - gainingPlayer.currentPoints)/totalDistanceFrom2000;
          gainingPlayer.notPlayedForPercentage =
            gainingPlayer.notPlayedForInverted/totalInvertedNotPlayedFor;
          gainingPlayer.fullPercentage =
            gainingPlayer.scorePercentage *
            gainingPlayer.notPlayedForPercentage;
          totalGainPercentage += gainingPlayer.fullPercentage;
        }

        // console.log("Total inverted not played for: " +
        // totalInvertedNotPlayedFor);
        ...
        console.log("Total Gains: " + totalActualGains +
          " Total decay: " + totalActualDecay);
        console.log(individualGains);
        console.log(individualDecay);

        // check if there are no runding errors, and if there are,
        // adjust for them
...
/**
 * Calculate and update database based on old rules.
 * If you win against someone above you, you move up
 * @param {FirebaseFirestore.CollectionReference} players List of players.
 * @param {number} winnerOldRank Rank of winning player
 */
export function calculateOldTable(
    players : FirebaseFirestore.CollectionReference,
 ...
 * Calculate and update database based on old rules.
 * If you win against someone above you, you move up
 * @param {Promise} promises Promises to calling function
 * @param {FirebaseFirestore.CollectionReference} players List of players
 * @param {number} gamesPlayed Total number of games played ever
 * @param {number} decayCalculationFactor Decay is calculated every x games
 * @param {number} decayCutoffGames If not playing for this time, decay happens
 * @param {number} decayPoints Points subtracted by decay
 */
export function decayPreparation(
    promises: Promise<void | FirebaseFirestore.WriteResult>[],
    players:FirebaseFirestore.CollectionReference,
    gamesPlayed:number,
